...
public:
    ...
    void backPropagation() {
    	...
    	for(int i=0;i<o;i++) {
    		for(int j=0;j<m;j++) {
    			w2[i][j] += eta*delta1[i]*hiddenLayer[j];
    		}
    	}
...
vector<vector<double>> purifyInput(string s) {
	...
	if(ss == "Iris-setosa") res.push_back({1.0,0,0});
	else if(ss == "Iris-versicolor") res.push_back({0,1.0,0});
	else res.push_back({0,0,1.0});
...
signed main() {
    ...
    for(int i=0;i<150;i++) {
    	accuracy += output[i][mlp.feedForward(input[i],output[i])];
    	/*cout<<mlp.outputLayer()<<enl;
    	cout<<output[i]<<enl;
    	cout<<input[i]<<enl;*/
    }
